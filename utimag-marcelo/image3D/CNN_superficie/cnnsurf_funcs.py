...
def crear_input_data(cfg, codepath, format, envelope=True):
    ...
    for i in adqs:
        ...
        prg.filt_kernel_int16(queue, matrix.shape[0:2], None, *args).wait()
        matrix_filt = buf['matrix_filt'].g2c()
        if envelope:
            args = utils.return_knl_args(buf, ['matrix_filt', 'hilb_coef', 'matrix_imag'])
            prg.filt_kernel(queue, matrix.shape[0:2], None, *args).wait()
        ...
        temp = temp[:, :, sli]
        input_data.append(temp.reshape(matrix.shape[0], 11, 11, n_samples_new))
...
def add_fliped_data(matrix):
    ...
    temp3 = np.flip(temp1, axis=2)  # flip doble, que genera una trasposicion?
    return np.concatenate((matrix, temp1, temp2, temp3), axis=0)
...
def return_offset_fun(matrix, t_idx, kernel):
    ...
    def fun(x):
        x = int(np.round(x))
        ofs_idx = np.clip(t_idx + x - hw, 0, windowed_signals.shape[2]-1)  # para evitar porblemas de borde
...
def return_example(model, i, fmc, u1, u2, peco_interval, sta_lta1, sta_lta2, window_num=10, w_min=0, vnet_thr_inverso=False):
    x = fmc[i, :, :, :, 0]
    # print(i)
    n_samples = x.shape[-1]
    ...
    y = fmc[i, :, :, :, 0]
    y = model(np.expand_dims(y, axis=0)).numpy()   # MAKING THE PREDICTION ON THE INPUT DATA
    y = y[0, :, :, :, 0].reshape((121, n_samples))
    if vnet_thr_inverso:
        y = 1 - np.flip(y, axis=0)
    idx2 = utils.first_thr_cross(y, [0, n_samples], u2, window_num)
    valid2 = idx2[:, 1] > w_min

    z = fmc[i, :, :, :, 0]
    if type(z) is not np.ndarray: # es tensor
        z = z.numpy()
    z = z.reshape((121, n_samples))
    ...
    idx3, _ = matched_filter(z, kernel, peco_interval)
    valid3 = idx3[:] > w_min

    t = fmc[i, :, :, :, 0]
    if type(t) is not np.ndarray: # es tensor
        t = t.numpy()
    t = t.reshape((121, n_samples))
    sta, lta = sta_lta1
    idx4, _, _ = sta_lta_fun(t, sta, lta, peco_interval)
    valid4 = idx4[:] > w_min

    sta2, lta2 = sta_lta2
    idx5, _, _ = sta_lta_fun(t, sta2, lta2, peco_interval)
    valid5 = idx5[:] > w_min

    return x, idx1[:, 0], valid1, y, idx2[:, 0], valid2, z, idx3[:], valid3, t, idx4[:], valid4, idx5[:], valid5


def return_example_ascan(i, fmc, peco_interval, sta_lta1, sta_lta2, thr):
    z = fmc[i, :, :, :, 0]
    n_samples = z.shape[-1]
    if type(z) is not np.ndarray:  # es tensor
        z = z.numpy()
    ...
    freq = 3; fs = 40; t0 = 0; bw = 0.8; t_max = 4 / freq  # Parámetros para pulso gaussiano
    kernel = utils.gaussian_pulse(freq, bw, t0, -t_max, t_max, fs)
    _, q = matched_filter(z, kernel, peco_interval)

    sta1, lta1 = sta_lta1
    _, cft1, cft1_dif = sta_lta_fun(z, sta1, lta1, peco_interval)

    sta2, lta2 = sta_lta2
    _, cft2, cft2_dif = sta_lta_fun(z, sta2, lta2, peco_interval)

    thr_curve, idx_cross = thr_cross_curve(z, thr, peco_interval)

    return z, q, cft1, cft2, cft1_dif, cft2_dif, thr_curve, idx_cross

def thr_cross_curve(fmc, thr, peco_interval):
    t1, t2 = peco_interval
    thr_curve = []; idx_cross = []
    fmc = np.abs(fmc)
    for i in range(fmc.shape[0]):
        thr_curve_i = np.zeros_like(fmc[i,:])
        thr_curve_i[t1:t2] = np.where(fmc[i, t1:t2] >= thr, 1, 0)
        thr_curve.append(thr_curve_i)
        #------------------------------------
        idx_cross_i = np.where(thr_curve_i[t1:t2] == 1)[0]
        if idx_cross_i.size > 0:
            first_idx_cross = idx_cross_i[0] + t1
        else:
            first_idx_cross = 0
        idx_cross.append(first_idx_cross)
        #------------------------------------
    thr_curve = np.array(thr_curve)
    idx_cross = np.array(idx_cross)

    return thr_curve, idx_cross

def matched_filter(fmc, kernel, peco_interval):
    # parametros para general el pulso gaussiano /  kernel para la correlacion
    q = correlate1d(fmc, kernel, axis=-1)  # hace la correlacion
...
def sta_lta_fun(fmc, sta, lta, peco_interval, fs=40):
    t_idx = []; cft = []; cft_dev = []
    t1, t2 = peco_interval
    for i in range(fmc.shape[0]):
        cft_i = recursive_sta_lta(fmc[i, :], int(sta * fs), int(lta * fs))
        # cft = classic_sta_lta(fmc[i, :], int(sta * fs), int(lta * fs))
        cft.append(cft_i)
        #-------------------getting the derivatrive of the signal ----------------
        cut_idx = 100
        cft_diff = np.diff(cft_i[cut_idx:])
        cft_r = np.zeros_like(cft_i)
        cft_r[cut_idx:cut_idx + len(cft_diff)] = cft_diff
        #-------------------------------------------------------------------------
        t_idx_i = np.argmax(np.abs(cft_r[t1:t2]), axis=-1) + t1
        t_idx.append(t_idx_i)
        cft_dev.append(cft_r)
    t_idx = np.array(t_idx)
    cft = np.array(cft)
    cft_dev = np.array(cft_dev)
    return t_idx, cft, cft_dev
